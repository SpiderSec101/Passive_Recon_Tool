...
def tool_file_download():
    print("[+] Downloading required files and tools")
...
def harvester_extract_data(input_file):
    hosts = set()
    ips = set()
    asns = set()
    emails = set()
    ...
    with open(input_file, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            for asn in asn_regex.findall(line):
                asns.add(asn.upper())
            ...
            if "." in line and "@" not in line:
                ...
                if re.fullmatch(r"[\*\w.-]+\.[A-Za-z]{2,}", host):
                    hosts.add(host)
    ...
    with open(f"{base_dir}/{target_domain}/assets/Domains/harvester.txt", "w") as f:
        f.write("\n".join(sorted(hosts)))

    with open(f"{base_dir}/{target_domain}/assets/IP/harvester.ip", "w") as f:
        f.write("\n".join(sorted(ips)))

    with open(f"{base_dir}/{target_domain}/assets/harvester.asn.txt", "w") as f:
        f.write("\n".join(sorted(asns)))

    with open(f"{base_dir}/{target_domain}/assets/secrets/harvester.emails", "w") as f:
        f.write("\n".join(sorted(emails)))
...
def harvester():
    ...
    harvester_extract_data(f"{base_dir}/{target_domain}/assets/harvester_output.txt")
